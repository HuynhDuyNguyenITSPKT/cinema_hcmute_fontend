...
function truncateText(text, maxLength = 140) {
  ...
  if (!normalized) return 'Thông tin khuyến mãi đang được cập nhật.'
...
function Promotions() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const response = await promotionService.getPublicPageable({ page, size: PAGE_SIZE, isActive: true })
  ...
  const summary = useMemo(() => {
    return `Hiện có ${pageMeta.totalItems} ưu đãi đang áp dụng.`
  }, [pageMeta.totalItems])
  ...
  return (
    <div className="container py-5">
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
          <h2 className="fw-bold text-light mb-1">🏷️ Khuyến Mãi</h2>
          <p className="text-secondary mb-0">{summary}</p>
        </div>
        <Link to="/movies" className="btn btn-outline-light">Xem lịch chiếu</Link>
      </div>

      {error ? <div className="alert alert-danger">{error}</div> : null}

      {loading ? (
        <div className="text-center text-secondary py-5">Đang tải danh sách khuyến mãi...</div>
      ) : null}

      {!loading && items.length === 0 && !error ? (
        <div className="text-center text-secondary py-5">Hiện chưa có khuyến mãi khả dụng.</div>
      ) : null}
      ...
      <div className="row g-4">
        {items.map((item) => {
          ...
          return (
            <div key={id || item.code || item.name} className="col-12 col-sm-6 col-lg-4">
              <article className="card h-100 bg-dark text-white border-secondary shadow-sm">
                ...
                <div className="card-body d-flex flex-column">
                  <div className="d-flex justify-content-between align-items-start gap-2 mb-2">
                    <h5 className="card-title mb-0">{item.name || 'Khuyến mãi chưa đặt tên'}</h5>
                    <span className="badge bg-success">{formatDiscount(item.discountType, item.discountValue)}</span>
                  </div>

                  <p className="mb-2">
                    <span className="badge bg-warning text-dark">Mã: {item.code || '-'}</span>
                  </p>
                  <p className="card-text text-white small mb-3">{truncateText(item.description)}</p>

                  <div className="small text-secondary mb-3">
                    <div>Bắt đầu: {formatDateTime(item.startDate)}</div>
                    <div>Kết thúc: {formatDateTime(item.endDate)}</div>
                  </div>
                </div>
              </article>
            </div>
